...
export function useStellarWallet(enabled: boolean) {
  ...
  const loadOrCreate = useCallback(async () => {
    ...
    setError(null);
    try {
      const existingPub = await SecureStore.getItemAsync(PUB_KEY_KEY);
      const existingSecret = await SecureStore.getItemAsync(SECRET_KEY_KEY);
      ...
      await SecureStore.setItemAsync(PUB_KEY_KEY, keypair.publicKey());
      await SecureStore.setItemAsync(SECRET_KEY_KEY, keypair.secret());
